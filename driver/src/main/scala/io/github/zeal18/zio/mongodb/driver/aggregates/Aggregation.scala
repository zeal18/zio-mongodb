    ...
    override def toBsonDocument[TDocument <: Object](
      ...
      def lookupStage(
        ...
        pipeline.foreach { stage =>
          codecRegistry
            ...
            .encode(
              writer,
              stage.toBson.toBsonDocument(documentClass, codecRegistry),
              context,
            )
        }
        writer.writeEndArray()
      ...
      self match {
        case Aggregation.Match(filter) =>
          simplePipelineStage("$match", filter.toBson)
        ...
        case Aggregation.Project(projection) =>
          simplePipelineStage("$project", projection.toBson)
