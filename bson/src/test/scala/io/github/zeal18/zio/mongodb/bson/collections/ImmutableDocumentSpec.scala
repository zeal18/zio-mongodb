...
class ImmutableDocumentSpec extends BaseSpec {
  ...
  it should "support multiple additions" in {
    ...
    doc1 should not be doc2
    doc2 should equal(
      Document("key" -> "value", "key2" -> "value2", "key3" -> "value3", "key4" -> "value4"),
    )
  }
  ...
  it should "provide values in the order set" in {
    doc.values.toSet should equal(
      Set(BsonString("value"), BsonString("value2"), BsonString("value3")),
    )
  ...
  it should "provide a valueSet in the order set" in {
    doc.valuesIterator.toSet should equal(
      Set(BsonString("value"), BsonString("value2"), BsonString("value3")),
    )
